      ...
      <div>
        <section className="py-20 bg-indigo-100">
          <div className="container max-w-6xl mx-auto">
            {courses ? (
              <>
                <h2 className="text-4xl font-bold tracking-tight text-center"> Enrolled Courses</h2>
                <p className="mt-2 text-lg text-center text-gray-600">Check out your list of enrolled courses below.</p>
              </>
            ) : (
              ""
            )}
            <div className="p-4 grid grid-cols-4 gap-8 mt-10 sm:grid-cols-8 lg:grid-cols-12 sm:px-8 xl:px-0">
              {courses ? (
                courses.map((course) => {
                  return (
                    <a className="duration-150 hover:scale-105 transform cursor-pointer hover:shadow-2xl relative flex flex-col items-center justify-between col-span-4 px-8 py-12 space-y-4 overflow-hidden bg-gray-100 rounded-xl text-gray-700">
                      ...
                      <div className="flex">
                        <Link key={course.ID} href={isStudentLoggedIn() === true ? "/courses/" + course.Slug : "/login?refer=/courses/" + course.Slug}>
                          <a className="bg-nirmaan hover:bg-nirmaan-dark text-gray-100 font-bold py-2 px-4 rounded inline-flex items-center">
                        ...
                        {/* <Link key={course.ID} href={isStudentLoggedIn() === true ? "/quizzes/" + course.Slug : "/login?refer=/courses/" + course.Slug}>
                          <a className="ml-2 bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center">
                            <span>Quiz</span>
                          </a>
                        </Link> */}
                      </div>
