...
if (isset($_GET)) {
    ...
    $student_data = $db->get('registrations', "*", "regID=$user_id");
    ...
    if (!empty($response)) {
        if ($response['Percentage'] >= 35) {
            header("Content-disposition: attachment; filename=\"" . basename('Certificate-' . $course_data['Title'] . '-' . $student_data['regID'] . ".png") . "\"");
            $name = strtoupper($student_data['first_name']);
            $name_len = strlen($name);
            $image = "basic-certificate.png";
            $createimage = imagecreatefrompng($image);
            $output = 'Certificate-' . $course_data['Title'] . '-' . $student_data['regID'] . ".png";
