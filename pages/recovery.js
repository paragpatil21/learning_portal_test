...
export default function Home() {
  ...
  function updatePassword(e) {
    ...
    if (password.trim() === "") {
      ...
      setLoginMessage({ type: "error", message: "Confirm password should be match with password.", icon: "error" });
    } else if (!/^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,16}$/.test(password)) {
      setLoginMessage({ type: "error", message: "Password: minimum eight characters, at least one number and one special character.", icon: "error" });
    } else {
      axios
