...
if (isset($_POST)) {
    ...
    if (!empty($user_id)) {
        ...
        if (!empty($level_data)) {
            ...
            $quiz_attempt = $db->count('certificate_status', 'ID', "CourseID=$level_id AND StudentID='$user_id'");
            $download_confirm = $db->get('certificate_confirm', 'Status', "CourseID=$level_id AND StudentID='$user_id'");
            $best_score = $db->get('certificate_status', "MAX(Percentage) as 'Percentage',CorrectAnswers,WrongAnswers", "StudentID='$user_id' AND CourseID=$level_id");
            ...
            $request->downloadConfirm = $download_confirm['Status'] ?? 'No';
            $request->bestScore = $best_score;
        } else {
            $request->meta = [
                "error" => true,
