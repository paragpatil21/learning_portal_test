...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!questionData && courseSlug) {
      axios
        .post(API_URL + "exercises/get_question.php", { course_slug: courseSlug, user_id: StudentData().UniqueID })
        .then((response) => {
  ...
  function submitAnswer(questionID) {
    ...
    setPageLoading(true);
    axios
      .post(API_URL + "exercises/get_question.php", { question_id: questionID, answer: answer_input, user_id: StudentData().UniqueID, course_slug: courseSlug })
      .then((response) => {
        setQuestionData(response.data.data);
        setQuizStatus(response.data.quizStatus);
        setQuizTotal(response.data.total);
  ...
  function ReplaceWithObject(textSource, textToReplace) {
    ...
    return { __html: html_text.replace("[[blank]]", "<input type='text' size='" + textToReplace.length + "' maxlength='" + textToReplace.length + "' id='answer' className='h-6 px-1 border-0' >") };
  }
  ...
  return pageLoading === false ? (
    <>
              ...
              <div className="p-10">
                ...
                <div className="py-5 px-8 space-y-3">
                  <div className="option1">
                    <span className="ml-2 text-gray-700 text-lg font-serif" dangerouslySetInnerHTML={ReplaceWithObject(questionData.Question, questionData.Answer)}></span>
                  </div>
                </div>
              </div>
              <button onClick={(e) => submitAnswer(questionData.ID)} className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center">
          ...
          {!quizStatus && quizTotal > 0 ? (
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-green-600 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex justify-center mb-10">
        <Link href="/quizzes">
          <a className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-1 px-4 rounded inline-flex items-center">
