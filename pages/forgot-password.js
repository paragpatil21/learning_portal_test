...
export default function Home() {
  ...
  function sendLink(e) {
    setLoginMessage({ type: "loading", message: "Request sent..please wait..", icon: "loading" });
    e.preventDefault();
    if (email.trim() === "") {
      setLoginMessage({ type: "error", message: "Please enter Email or Registration ID.", icon: "error" });
    } else {
      axios
        ...
        <div className="bg-white py-24 px-4 overflow-hidden sm:px-6 lg:px-8 lg:py-32">
          <div className="relative max-w-lg mx-auto">
            ...
            <div className="text-center">
              <h2 className="text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">Forgot Password?</h2>
            </div>
            <div className="mt-6">
              <form action="#" method="POST" className="grid grid-cols-1 gap-y-6 sm:grid-cols-2 sm:gap-x-8">
                <div className="sm:col-span-2">
                  ...
                  <div className="mt-1">
                    <input type="email" name="email" id="email" autoComplete="email" required onChange={(e) => setEmail(e.target.value)} className="py-3 px-4 block w-full shadow-sm focus:ring-nirmaan focus:border-nirmaan border-gray-300 rounded-md" placeholder="Enter Email or Registration ID" />
                  </div>
                </div>
